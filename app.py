...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE, force_text=None):
    ...
    if "Последние 10" in text:
        ...
        doc_text = (
            ...
            " один из первых нормальных ботов, которые я написал. Сделано 31.12.2025. Я считаю этого бота отправной точкой в своей инженерной карьере. Это моя первая реально значимая(в моих глазах) автоматизация, робота над своей системой"
        )
